...
public class Item2DRenderer {
    public static final String[] HAND_MODEL_ITEMS = new String[]{"static_rifle","whirlwind_spear"};

    @SubscribeEvent
    public static void onModelBakeEvent(ModelEvent.ModifyBakingResult event) {
        Map<ResourceLocation, BakedModel> map = event.getModels();
        for (String item : HAND_MODEL_ITEMS) {
            System.out.println("!!!BAKING "+item);
            ResourceLocation modelInventory = new ModelResourceLocation(new ResourceLocation(VividLight.MOD_ID, item), "inventory");
            ResourceLocation modelHand = new ModelResourceLocation(new ResourceLocation(VividLight.MOD_ID, item + "_in_hand"), "inventory");
